...
> 本人的第一个完整后端项目 —— 基于 Spring Boot 3 构建的用户注册、登录与管理系统。

------

## 📖 项目简介

用户管理中心（User Center）是一个基于 **Spring Boot 3** 的 RESTful 后端服务，提供用户注册、登录、信息查询等核心功能。项目集成了 MyBatis-Plus 简化数据库操作，并使用 Knife4j 提供在线 API 文档，方便接口调试与对接。**本项目严格遵循企业级开发规范，注重代码分层、安全性与高内聚低耦合设计。**
...
2. 严密的数据安全防护：
	...
	- **传输安全**：严格规范 `DTO` (如 `UserRegisterRequest`) 与 `Entity` 的实体分层，实现前端请求数据与底层数据模型的物理隔离；并在接口层重写 `getSafetyUser` 方法，对密码等核心隐私数据进行强制脱敏。
3. 高内聚的代码组织：
...
或者在 IDEA中直接运行 `UserCenterApplication.java` 主类。
